 ...
 *   quickHull.Point3d[] points = new quickHull.Point3d[]
 *    { new quickHull.Point3d (0.0,  0.0,  0.0),
 *      new quickHull.Point3d (1.0,  0.5,  0.0),
 *      new quickHull.Point3d (2.0,  0.0,  0.0),
 *      new quickHull.Point3d (0.5,  0.5,  0.5),
 *      new quickHull.Point3d (0.0,  0.0,  2.0),
 *      new quickHull.Point3d (0.1,  0.2,  0.3),
 *      new quickHull.Point3d (0.0,  2.0,  0.0),
 *    };
 *
 *   quickHull.QuickHull3D hull = new quickHull.QuickHull3D();
 *   hull.build (points);
 *
 *   System.out.println ("Vertices:");
 *   quickHull.Point3d[] vertices = hull.getVertices();
 *   for (int i = 0; i < vertices.length; i++)
 *    { quickHull.Point3d pnt = vertices[i];
 ...
 * tested quickHull.QuickHull3D for such situations by computing the convex hull of a
 ...
 * trials and so we are confident that quickHull.QuickHull3D is reasonably robust.
 *
 * <h3>Merged Faces</h3> The merging of faces means that the faces returned by
 * quickHull.QuickHull3D may be convex polygons instead of triangles. If triangles are
...
public class QuickHull3D {
     ...
     */
    private static final int INDEXED_FROM_ZERO = 0x4;
    ...
    private final Vector faces = new Vector(16);
    private final Vector horizon = new Vector(16);
    ...
    private void setHull(double[] coords, int nump,
     ...
     *                                  coplanar.
     */
    private void build(double[] coords, int nump)
            throws IllegalArgumentException {
        if (nump < 4) {
...
// 	private void splitFace (quickHull.Face face)
// 	 {
//  	   quickHull.Face newFace = face.split();
//  	   if (newFace != null)
...
// 	 }
     ...
     */
    private void build(Point3d[] points, int nump)
            throws IllegalArgumentException {
        if (nump < 4) {
            throw new IllegalArgumentException(
                    "Less than four input points specified");
     ...
     * @see QuickHull3D#getVertices()
     */
    private int[][] getFaces(int indexFlags) {
...
//	   oldFaces.add (face);
    ...
    private boolean checkFaceConvexity(
