...
func main() {
	...
	ui := &UI{
		...
		code:          sh.RandCode(),
		sh_:           sh.NewSH(512, rand.Intn(25), sh.RandCode()),
		multiThreaded: true,
	}
	ui.update = func() {
		t0 := time.Now()
...
func (ui *UI) keyCallback(w *glfw.Window, key glfw.Key, scancode int, action glfw.Action, mods glfw.ModifierKey) {
	if action == glfw.Press {
		...
		case glfw.KeyLeft: // colormap +-
			ui.colorMap = (ui.colorMap + 1) % sh.N_COLOR_MAPS

		case glfw.KeyRight:
			ui.colorMap = (ui.colorMap + sh.N_COLOR_MAPS - 1) % sh.N_COLOR_MAPS

		case glfw.KeyM: // multi thread
			ui.multiThreaded = !ui.multiThreaded

		case glfw.KeyT: // test worky code
			ui.code = sh.FindCode(88888888)

		case glfw.KeyW: // write obj
			ui.sh_.WriteObj(fmt.Sprintf("sh_%d_%d_%d.obj", ui.res, ui.colorMap, sh.SH_codes[ui.code]))
			return
		...
		ui.sh_ = sh.NewSH(ui.res, ui.colorMap, ui.code)
		ui.update()
	}
}
